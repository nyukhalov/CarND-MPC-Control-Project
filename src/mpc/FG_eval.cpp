...
void FG_eval::operator()(ADvector &fg, const ADvector &vars)
{
  ...
  for (size_t t = 0; t < config.num_states; ++t)
  {
    ...
    if (t > 1)
    {
      AD<double> y_prev = vars[t - 1 + config.y_start];
      AD<double> y_prev_prev = vars[t - 2 + config.y_start];
      AD<double> y_vel = y - y_prev;
      AD<double> y_vel_prev = y_prev - y_prev_prev;
      AD<double> y_accel = y_vel - y_vel_prev;
      fg[0] += 100 * CppAD::pow(y_accel, 2);
    }
  }
